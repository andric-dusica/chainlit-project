    ...
    <div className={`group flex items-center ${isUser ? 'justify-end' : 'justify-start'}`}>
      {isUser && !isEditing && (
        <div className="relative tooltip-wrapper">
            <button
            className="mr-2 p-2 rounded-md focus:outline-none tooltip-btn opacity-0 group-hover:opacity-100 transition-opacity"
            ...
            <span
            className={`absolute bottom-[-40px] !left-0 transform -translate-x-1/2 opacity-0 pointer-events-none transition-opacity duration-300 tooltip ${
        ...
        {isEditing ? (
          <div>
            ...
            <div className="flex gap-2 mt-1">
              <button className="text-green-600 hover:underline" onClick={handleSaveClick}>
                {t('buttons.save')}
              </button>
              <button className="text-red-600 hover:underline" onClick={handleCancelClick}>
                {t('buttons.cancel')}
              </button>
            </div>
          </div>
            ...
            {message.sender !== 'user' && (
              <div className="flex items-center mt-2 space-x-2">
                ...
                {showCopied && <span className="text-xs text-green-500">{t('copy.copied')}</span>}
              </div>
            )}
          </div>
